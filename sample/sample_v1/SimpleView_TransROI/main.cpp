...
int main(int argc, char* argv[])
{
    ...
    if (allComps & TY_COMPONENT_DEPTH_CAM && depth) {
        ...
        int32_t t_roi_x = TYImageWidth(image_mode) / 4;
        int32_t t_roi_y = TYImageHeight(image_mode) / 4;
        int32_t t_roi_w = TYImageWidth(image_mode) / 2;
        int32_t t_roi_h = TYImageHeight(image_mode) / 2;
        
        TYSetInt(hDevice, TY_COMPONENT_DEPTH_CAM, TY_INT_TRANSMIT_ROI_OFFSET_X, t_roi_x);
        TYSetInt(hDevice, TY_COMPONENT_DEPTH_CAM, TY_INT_TRANSMIT_ROI_OFFSET_Y, t_roi_y);
        TYSetInt(hDevice, TY_COMPONENT_DEPTH_CAM, TY_INT_TRANSMIT_ROI_WIDTH, t_roi_w);
        TYSetInt(hDevice, TY_COMPONENT_DEPTH_CAM, TY_INT_TRANSMIT_ROI_HEIGHT, t_roi_h);
        TYSetBool(hDevice, TY_COMPONENT_DEPTH_CAM, TY_BOOL_TRANSMIT_ROI_ENABLE, 1);
