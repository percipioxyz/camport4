#include "../common/common.hpp"
#include <iostream>
#include <thread>
#include <chrono>


#if _WIN32
#include <conio.h>
#elif __linux__
#include <termio.h>
#include <vector>
#define TTY_PATH    "/dev/tty"
#define STTY_DEF    "stty -raw echo -F"
#endif

TY_DEV_HANDLE hDevice = NULL;

void eventCallback(TY_EVENT_INFO *event_info, void *userdata)
{
    if (event_info->eventId == TY_EVENT_DEVICE_OFFLINE) {
        LOGD("=== Event Callback: Device Offline!");
        // Note: 
        //     Please set TY_BOOL_KEEP_ALIVE_ONOFF feature to false if you need to debug with breakpoint!
    }
    else if (event_info->eventId == TY_EVENT_LICENSE_ERROR) {
        LOGD("=== Event Callback: License Error!");
    }
    else if (event_info->eventId == TY_EVENT_CMD)
    {
        LOGD("=== Event Callback: %s", event_info->message);
        std::string categoryName(event_info->message);
        size_t pos = categoryName.find("Port");
        categoryName.replace(pos, 4, "Data");

        uint32_t cnt;
        TYGetFeaturesCount(hDevice, categoryName.c_str(), &cnt);
        if (cnt == 0)
            return;

        std::vector<TY_GENI_FEATURE_NAME> features(cnt);
        TYGetFeatures(hDevice, categoryName.c_str(), features.data(), cnt, &cnt);
        for (int i = 0; i < cnt; i++) {
            int64_t value = 0;
            TYIntegerGetValue(hDevice, features[i], &value);
            LOGD("%s: %" PRId64 "", features[i], value);
        }
        printf("\n");
    }
}

int main(int argc, char* argv[])
{
    std::string ID, IP;
    TY_INTERFACE_HANDLE hIface = NULL;
    uint32_t event_test_mode = 0;
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-id") == 0) {
          ID = argv[++i];
        } else if (strcmp(argv[i], "-ip") == 0) {
          IP = argv[++i];
        } else if (strcmp(argv[i], "-t") == 0) {
          event_test_mode = std::stoul(argv[++i]);
        }
    }

    LOGD("Init lib");
    ASSERT_OK( TYInitLib() );
    TY_VERSION_INFO ver;
    ASSERT_OK( TYLibVersion(&ver) );
    LOGD("     - lib version: %d.%d.%d", ver.major, ver.minor, ver.patch);

    std::vector<TY_DEVICE_BASE_INFO> selected;
    ASSERT_OK( selectDevice(TY_INTERFACE_ALL, ID, IP, 1, selected) );
    ASSERT(selected.size() > 0);
    TY_DEVICE_BASE_INFO& selectedDev = selected[0];

    ASSERT_OK( TYOpenInterface(selectedDev.iface.id, &hIface) );
    ASSERT_OK( TYOpenDevice(hIface, selectedDev.id, &hDevice) );

    ASSERT_OK(TYEnumSetString(hDevice, "SourceSelector", "Texture"));
    ASSERT_OK(TYBooleanSetValue(hDevice, "ComponentEnable", true));
    ASSERT_OK(TYEnumSetString(hDevice, "SourceSelector", "Depth"));
    ASSERT_OK(TYBooleanSetValue(hDevice, "ComponentEnable", false));
    ASSERT_OK(TYEnumSetString(hDevice, "SourceSelector", "Right"));
    ASSERT_OK(TYBooleanSetValue(hDevice, "ComponentEnable", false));
    ASSERT_OK(TYEnumSetString(hDevice, "SourceSelector", "Left"));
    ASSERT_OK(TYBooleanSetValue(hDevice, "ComponentEnable", false));

    LOGD("Register event callback");
    ASSERT_OK(TYRegisterEventCallback(hDevice, eventCallback, NULL));
    LOGD("TestEventMode %d", event_test_mode);
    if (event_test_mode == 0) {
        LOGD("TestEventMode WithOutData");
        while (1) {
            ASSERT_OK(TYEnumSetString(hDevice, "TestEventMode", "WithOutData"));
            TYCommandExec(hDevice, "TestEventGenerate");
            std::this_thread::sleep_for(std::chrono::seconds(3));
        }
    } else if (event_test_mode == 1) {
        LOGD("TestEventMode WithData");
        while (1) {
            ASSERT_OK(TYEnumSetString(hDevice, "TestEventMode", "WithData"));
            TYCommandExec(hDevice, "TestEventGenerate");
            std::this_thread::sleep_for(std::chrono::seconds(3));
        }
    } else if (event_test_mode == 2) {
        LOGD("TestEventMode Burst");
        while (1) {
            ASSERT_OK(TYEnumSetString(hDevice, "TestEventMode", "Burst"));
            TYCommandExec(hDevice, "TestEventGenerate");
            std::this_thread::sleep_for(std::chrono::seconds(3));
        }
    } else {
        LOGD("Trigger Event");
        ASSERT_OK(TYEnumSetString(hDevice, "EventSelector", "FrameTrigger"));
        ASSERT_OK(TYEnumSetString(hDevice, "EventNotification", "On"));

        ASSERT_OK(TYEnumSetString(hDevice, "EventSelector", "FrameTriggerMissed"));
        ASSERT_OK(TYEnumSetString(hDevice, "EventNotification", "On"));
    }

    bool exit_main = false;
    std::vector<char> frameBuffer[2];
    if (!exit_main) {
        LOGD("Prepare image buffer");
        uint32_t frameSize;
        ASSERT_OK( TYGetFrameBufferSize(hDevice, &frameSize) );
        LOGD("     - Get size of framebuffer, %d", frameSize);

        LOGD("     - Allocate & enqueue buffers");
        
        frameBuffer[0].resize(frameSize);
        frameBuffer[1].resize(frameSize);
        LOGD("     - Enqueue buffer (%p, %d)", frameBuffer[0].data(), frameSize);
        ASSERT_OK( TYEnqueueBuffer(hDevice, frameBuffer[0].data(), frameSize) );
        LOGD("     - Enqueue buffer (%p, %d)", frameBuffer[1].data(), frameSize);
        ASSERT_OK( TYEnqueueBuffer(hDevice, frameBuffer[1].data(), frameSize) );

        LOGD("Start capture");
        ASSERT_OK( TYStartCapture(hDevice) );

        LOGD("While loop to fetch frame");
        TY_FRAME_DATA frame;
        int index = 0;
        double depth_scale_unit = 1.0;
        while(!exit_main) {
            ASSERT_OK(TYCommandExec(hDevice, "TriggerSoftware"));
            int err = TYFetchFrame(hDevice, &frame, -1);
            if( err == TY_STATUS_OK ) {
                LOGD("Get frame %d", ++index);

                float fps = get_fps();
                if (fps > 0){
                    LOGI("fps: %.2f", fps);
                }

                for (int i = 0; i < frame.validCount; i++){
                    if (frame.image[i].status != TY_STATUS_OK) continue;
                    decode_and_display_image(frame.image[i], depth_scale_unit);
                }
                int key = TYWaitKeyEvents();
                switch(key & 0xff) {
                case 0xff:
                    break;
                case 'q':
                    exit_main = true;
                    break;
                default:
                    LOGD("Unmapped key %d", key);
                }

                LOGD("Re-enqueue buffer(%p, %d)"
                    , frame.userBuffer, frame.bufferSize);
                ASSERT_OK( TYEnqueueBuffer(hDevice, frame.userBuffer, frame.bufferSize) );
            }

        }
        ASSERT_OK( TYStopCapture(hDevice) );
        //stop will not release all buffers, need clear
        ASSERT_OK( TYClearBufferQueue(hDevice) );

    }

    ASSERT_OK( TYCloseDevice(hDevice));
    ASSERT_OK( TYCloseInterface(hIface) );
    ASSERT_OK( TYDeinitLib() );
    LOGD("Main done!");
    return 0;
}
