...
void parseImageWaterMark(const TY_IMAGE_DATA *img) {
    uint32_t WMVer = 0;
    //means no waterMark or img is invalid if ret val is not TY_STATUS_OK
    if (TY_STATUS_OK == TYGetImageWatermarkVer(img, &WMVer)) {
        //can use WMVer >= WATERMARKV1 here if you don't care new info in new version
        if (WMVer == WATERMARKV1) {
          TY_IMAGE_WATER_MARK wm;
          memset(&wm, 0, sizeof(wm));
          TYGetImageWatermark(img, &wm, sizeof(wm));
          LOGD("expo %f, undis %s\n", wm.exposure, wm.undistortion == 0 ? "false" : "true");
        }   
        /*  
        //handle other version
        else if (WMVer == WATERMARKV2){
        }   
        */  
    }
}
    ...
    while(!exit_main) {
        ...
        if( err == TY_STATUS_OK ) {
            ...
            for (int i = 0; i < frame.validCount; i++){
                parseImageWaterMark(&frame.image[i]);
